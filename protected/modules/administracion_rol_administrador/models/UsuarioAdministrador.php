 ...
 * @property EscritorioAdministrador[] $escritorioAdministradors
...
class UsuarioAdministrador extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
			'escritorioAdministradors' => array(self::HAS_MANY, 'EscritorioAdministrador', 'usuario_administrador_id'),
			'rolAdministradors' => array(self::MANY_MANY, 'RolAdministrador', 'usuario_administrador_has_rol_administrador(usuario_administrador_id, rol_administrador_id)'),
		);
	}
	...
	public static function model($className=__CLASS__)
	{
        ...
        public function listarPermisoRol($nuevo_rol_id) {
            
             $comando = Yii::app()->db->createCommand("CALL listar_permiso_rol(:nuevo_rol_id)");
             $comando->bindParam(':nuevo_rol_id',$nuevo_rol_id);
             return $comando->queryAll();
        }
        public function desasignarPermisoUsuario($nuevo_usuario_id, $nuevo_permiso_name) {
            
            $comando = Yii::app()->db->createCommand("CALL desasignar_permiso_usuario(:nuevo_usuario_id,:nuevo_permiso_name)");
            $comando->bindParam(':nuevo_usuario_id',$nuevo_usuario_id);
            $comando->bindParam(':nuevo_permiso_name',$nuevo_permiso_name);
            $comando->execute();
            return $comando;
        }
        public function asignarPermisoUsuario($nuevo_usuario_id,$nuevo_permiso_name) {
            
            $comando = Yii::app()->db->createCommand("CALL asignar_permiso_usuario(:nuevo_usuario_id,:nuevo_permiso_name)");
            $comando->bindParam(':nuevo_usuario_id',$nuevo_usuario_id);
            $comando->bindParam(':nuevo_permiso_name',$nuevo_permiso_name);
            $comando->execute();
            return $comando;
