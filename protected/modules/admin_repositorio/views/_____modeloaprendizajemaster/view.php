<?php
/* @var $this ModeloaprendizajemasterController */
/* @var $model ModeloAprendizajeMaster */

$this->breadcrumbs=array(
	'Modelo Aprendizaje Masters'=>array('index'),
	$model->id,
);

$this->menu=array(
	array('label'=>'List ModeloAprendizajeMaster', 'url'=>array('index')),
	array('label'=>'Create ModeloAprendizajeMaster', 'url'=>array('create')),
	array('label'=>'Update ModeloAprendizajeMaster', 'url'=>array('update', 'id'=>$model->id)),
	array('label'=>'Delete ModeloAprendizajeMaster', 'url'=>'#', 'linkOptions'=>array('submit'=>array('delete','id'=>$model->id),'confirm'=>'Are you sure you want to delete this item?')),
	array('label'=>'Manage ModeloAprendizajeMaster', 'url'=>array('admin')),
);
?>

<h1>View ModeloAprendizajeMaster #<?php echo $model->id; ?></h1>

<?php $this->widget('zii.widgets.CDetailView', array(
	'data'=>$model,
	'attributes'=>array(
		'id',
		'nombre',
		'descripcion',
		'fecha_acceso',
		'fecha_modificacion',
		'fecha_creacion',
	),
)); ?>
